...
//! template, validates the committed fleet task-graph snapshots, and enforces the
//! closure, owner-fan-out, aggregation, gate, and routing invariants. Any one-byte
//! hand edit to a generated file — including a neutered composite run-script body —
//! fails the audit.
...
/// Directory of committed per-repository mise task-graph snapshots, relative to
/// the fleet root. Regenerated by `mise run fleet-lint`
/// (crates/velnor-actions-fleetlint).
const TASK_GRAPH_DIR: &str = "fleet/task-graphs";
...
/// gate, task-graph snapshot, or routing violation.
pub fn audit(root: &Path) -> Result<String, String> {
    audit_with_options(root, false)
}

/// Run the full fleet audit, optionally skipping the fleet task-graph snapshot
/// checks (`--skip-fleetlint` for offline development). Live task-graph
/// re-verification requires the fleet repository clones and the
/// `velnor-actions-fleetlint` binary; when either is absent the audit prints a
/// visible warning and validates the committed snapshots only — it never passes
/// silently.
///
/// # Errors
///
/// Returns `Err` on any contract, byte, closure, owner-fan-out, aggregation,
/// gate, task-graph snapshot, or routing violation.
pub fn audit_with_options(root: &Path, skip_fleetlint: bool) -> Result<String, String> {
    ...
    if !skip_fleetlint {
        audit_fleet_task_graphs(root, &manifest)?;
    }

    let class_gates: usize = manifest
        .classes()
        .iter()
        .map(|class| class.gates.len())
        .sum();
    let scheduled_gates: usize = manifest
        .repositories()
        .iter()
        .map(|repo| repo.gates.len())
        .sum();
    Ok(format!(
        "fleet valid: {} repositories, {} classes, {class_gates} class gates, {scheduled_gates} scheduled gates, {} templates",
...
/// Validate the committed fleet task-graph snapshots: one per member, byte-bound
/// to the member's scheduled gates, with zero findings. When the fleet clones
/// are present, re-run the linter binary against them live.
fn audit_fleet_task_graphs(root: &Path, manifest: &FleetManifest) -> Result<(), String> {
    let dir = root.join(TASK_GRAPH_DIR);
    ...
    if let Ok(entries) = std::fs::read_dir(&dir) {
        for entry in entries {
            ...
            if name.to_string_lossy().ends_with(".json") {
                unknown.insert(name.to_string_lossy().into_owned());
            }
        }
    }

    for repo in manifest.repositories() {
        let name = format!("{}.json", repo.slug.replace('/', "-"));
        if !unknown.remove(&name) {
            return Err(format!(
                "{TASK_GRAPH_DIR}/{name} is missing for fleet member {} (run `mise run fleet-lint`)",
                repo.slug
            ));
        }
        let path = dir.join(&name);
        let text = read_committed(&path)?;
        let snapshot: serde_json::Value = serde_json::from_str(&text).map_err(|e| {
            format!(
                "{}: parsing task-graph snapshot: {e} (run `mise run fleet-lint`)",
                path.display()
            )
        })?;
        if snapshot["slug"].as_str() != Some(repo.slug.as_str())
            || snapshot["class"].as_str() != Some(repo.class.code())
        {
            return Err(format!(
                "{}: snapshot identity does not match fleet member {}",
                path.display(),
                repo.slug
            ));
        }
        let scheduled = snapshot["scheduled_gates"].as_array();
        let scheduled = scheduled
            .map(|gates| {
                gates
                    .iter()
                    .map(|gate| gate.as_str().unwrap_or_default().to_string())
                    .collect::<Vec<_>>()
            })
            .unwrap_or_default();
        if scheduled != repo.gates {
            return Err(format!(
                "{}: snapshot schedules {scheduled:?} but the manifest schedules {:?} (run `mise run fleet-lint`)",
                path.display(),
                repo.gates
            ));
        }
        if snapshot["reachable_leaves"].as_array().is_none() {
            return Err(format!(
                "{}: snapshot has no reachable_leaves array (run `mise run fleet-lint`)",
                path.display()
            ));
        }
        ...
        {
            return Err(format!(
                "{}: task-graph findings are present — resolve them in the member's mise task graph (see `mise run fleet-lint`)",
                path.display()
            ));
        }
    }
    if !unknown.is_empty() {
        return Err(format!(
            "{TASK_GRAPH_DIR} contains snapshots for unknown members: {}",
            unknown.iter().cloned().collect::<Vec<_>>().join(", ")
        ));
    }

    // Live re-verification: possible only with the fleet clones and the linter
    // binary. Absence is warned about loudly, never silently passed over.
    let repos_dir =
        std::env::var_os("VELNOR_FLEET_REPOS_DIR").map_or_else(|| root.join(".."), PathBuf::from);
    if !repos_dir.is_dir() {
        eprintln!(
            "fleet lint: repositories directory {} not present; committed task-graph snapshots validated only (set VELNOR_FLEET_REPOS_DIR or run `mise run fleet-lint`)",
            repos_dir.display()
        );
        return Ok(());
    }
    let Some(binary) = fleetlint_binary(root) else {
        eprintln!(
            "fleet lint: repositories present at {} but no velnor-actions-fleetlint binary was found (build the workspace or set VELNOR_FLEETLINT_BIN); committed task-graph snapshots validated only",
            repos_dir.display()
        );
        return Ok(());
    };
    let output = Command::new(&binary)
        ...
        .arg("--check")
        .output()
        .map_err(|e| format!("executing {}: {e}", binary.display()))?;
    if !output.status.success() {
        return Err(format!(
            "fleet task-graph lint failed against {}:\n{}",
            binary.display(),
...
/// Locate the fleet linter binary: an explicit override first, then the usual
/// cargo build outputs inside the fleet root.
fn fleetlint_binary(root: &Path) -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("VELNOR_FLEETLINT_BIN") {
