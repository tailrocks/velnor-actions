...
if [[ -n "${RECOVERY_PROOF_ID}" ]]; then
  ...
  matching_prs="$(gh api "repos/${REPOSITORY}/pulls?state=open&per_page=100" --paginate --slurp --jq "map(.[]) | map(select(.head.sha == \"${HEAD_SHA}\")) | length")"
  ...
  duplicate_count="$(gh api "repos/${REPOSITORY}/actions/runs?event=workflow_dispatch&per_page=100" --paginate --slurp --jq "map(.workflow_runs[]) | map(select(.display_title == \"CI recovery ${RECOVERY_PROOF_ID}\" and .head_sha == \"${HEAD_SHA}\")) | length")"
