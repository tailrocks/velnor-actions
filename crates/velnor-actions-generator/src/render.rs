...
use crate::model::{ClassContract, Gate, Lane};
...
pub fn consumer_template(class: RepositoryClass) -> String {
    consumer_template_for(class, &ForkTable::canonical())
}

/// Render a consumer template from the validated owner-local fork table.
#[must_use]
pub fn consumer_template_for(class: RepositoryClass, forks: &ForkTable) -> String {
    ...
    for fork in forks.forks() {
        let variable = fork.job_id().to_ascii_uppercase();
        b.line(
            10,
            &format!(
                "{variable}_RESULT: ${{{{ needs.{}.result }}}}",
                fork.owner()
            ),
        );
        b.line(
            10,
            &format!(
                "{variable}_CONTRACT: ${{{{ needs.{}.outputs.contract }}}}",
                fork.owner()
            ),
        );
    }
    b.line(8, "shell: bash");
    b.run_block(8, &ci_required_script(forks));
...
pub fn callable_workflow(
    ...
    callable_workflow_for(contract, caches, block_sha, &ForkTable::canonical())
}

/// Render a callable workflow from the validated owner-local fork table.
#[must_use]
pub fn callable_workflow_for(
    contract: &ClassContract,
    caches: &CacheContract,
    block_sha: &str,
    forks: &ForkTable,
    ...
    b.line(
        ...
        &format!(
            ...
            hex::encode(Sha256::digest(
                consumer_template_for(class, forks).as_bytes()
            ))
        ),
    );
    ...
    b.run_block(8, &validate_request_script(forks));
    b.blank();
        ...
        for cache in &declarations {
            ...
            if lane == "velnor" {
                ...
                b.line(
                    8,
                    &format!(
                        "uses: {}/{ACTIONS_REPO}/actions/cache-contract@{block_sha}",
                        run_gate.split('/').next().unwrap_or(CANONICAL_OWNER)
                    ),
                );
                b.line(8, "with:");
    ...
    if lane == Lane::Velnor {
            ...
            .filter(|cache| cache.class == contract.class)
        {
            ...
            b.line(
                8,
                &format!(
                    "uses: {}/{ACTIONS_REPO}/actions/cache-contract@{block_sha_placeholder}",
                    run_gate.split('/').next().unwrap_or(CANONICAL_OWNER),
                    block_sha_placeholder = run_gate.rsplit_once('@').map_or("", |(_, sha)| sha)
                ),
            );
            b.line(8, "with:");
            b.line(10, "schema-version: 1");
...
const VALIDATE_REQUEST_SCRIPT_TEMPLATE: &str = r#"set -euo pipefail
...
is_benchmark_generation() {
  ...
  mapfile -t pin_rows < <(sed -nE 's~^    uses: (@OWNER_PATTERN@)/velnor-actions/\.github/workflows/ci-[a-z-]+\.yml@([0-9a-f]{40}) # (20[0-9]{2}\.[0-9]+\.[0-9]+)$~\1\t\2\t\3~p' "${workflow_file}")
  [[ "${#pin_rows[@]}" == @FORK_COUNT@ ]] || fail "caller owner pin cardinality mismatch"
  owners="$(printf '%s\n' "${pin_rows[@]}" | cut -f1 | LC_ALL=C sort -u | paste -sd, -)"
  [[ "${owners}" == '@SORTED_OWNERS@' ]] || fail "caller owner pin set mismatch"
...
@OWNER_PLACEHOLDER_CASES@
  ...
  [[ "$(grep -Ec '@[A-Z0-9_]+@ # @CALVER@' "${normalized_file}")" == @FORK_COUNT@ ]] || fail "caller normalization count mismatch"
...
/// Render the fail-closed request validator with owner data from the fork table.
#[must_use]
pub fn validate_request_script(forks: &ForkTable) -> String {
    let owner_pattern = forks
        .forks()
        .iter()
        .map(|fork| fork.owner())
        .collect::<Vec<_>>()
        .join("|");
    let owner_list = forks
        .forks()
        .iter()
        .map(|fork| fork.owner())
        .collect::<Vec<_>>()
        .join(" ");
    let sorted_owners = forks.sorted_owners().join(",");
    let placeholder_cases = forks
        .forks()
        .iter()
        .map(|fork| {
            format!(
        ...
        .collect::<Vec<_>>()
        .join("\n");
    VALIDATE_REQUEST_SCRIPT_TEMPLATE
        .replace("@OWNER_PATTERN@", &owner_pattern)
        .replace("@OWNER_LIST@", &owner_list)
        .replace("@SORTED_OWNERS@", &sorted_owners)
        .replace("@OWNER_PLACEHOLDER_CASES@", &placeholder_cases)
        .replace("@FORK_COUNT@", &forks.len().to_string())
}
...
pub fn render_consumer(
    ...
    render_consumer_for(template, &ForkTable::canonical(), &release_shas, calver)
}

/// Materialize a consumer template using the release pins ordered by the fork table.
pub fn render_consumer_for(
    template: &str,
    forks: &ForkTable,
    release_shas: &[&str],
    calver: &str,
) -> Result<String, String> {
    if release_shas.len() != forks.len() {
        return Err(format!(
            "expected {} owner release SHAs, got {}",
            forks.len(),
            release_shas.len()
        ));
    }
    ...
    for (fork, sha) in forks.forks().iter().zip(release_shas.iter().copied()) {
    ...
    if forks
        .forks()
        .iter()
        .any(|fork| out.contains(fork.placeholder()))
...
fn ci_required_script(forks: &ForkTable) -> String {
    let mut script = String::from(
        "set -euo pipefail\n# Positive truth table: accept only one recognized owner with an explicit ",
    );
    script.push_str(
        "contract=success output; non-selected owners must be skipped with empty outputs. ",
    );
    script.push_str(
        r#"The selected owner must be 'success'; non-selected owners must be expected both 'skipped' and expected empty.
case "${OWNER}" in
"#,
    );
    for fork in forks.forks() {
        let variable = fork.job_id().to_ascii_uppercase();
        script.push_str(&format!(
            "  {}) selected_result=\"${{{variable}_RESULT}}\"; selected_contract=\"${{{variable}_CONTRACT}}\" ;;\n",
            fork.owner()
        ));
    }
    script.push_str(
...
"#,
    ...
    script.push_str(
        r#"if [ "${selected_result}" != "success" ]; then
...
"#,
    ...
    for fork in forks.forks() {
        let variable = fork.job_id().to_ascii_uppercase();
        script.push_str(&format!(
            "if [ \"${{OWNER}}\" != \"{}\" ]; then\n  if [ \"${{{variable}_RESULT}}\" != \"skipped\" ]; then echo \"ci-required: non-selected owner {} ran, expected both 'skipped'\" >&2; exit 1; fi\n  if [ -n \"${{{variable}_CONTRACT}}\" ]; then echo \"ci-required: non-selected owner {} produced a contract output, expected empty\" >&2; exit 1; fi\nfi\n",
            fork.owner(),
            fork.owner(),
            fork.owner()
        ));
    }
    script.push_str(r#"echo "ci-required: fleet contract satisfied for owner '${OWNER}'""#);
    script
}
